...
@Component({
  selector: 'pie-chart',
  template: `
    <div id="{{random}}" style='width:100%;position:absolute;x=0;y=0;' (window:resize)="listenWindow()">
        <!--a>download xyz</a>
        <canvas id='asdfghcc'></canvas>
        <img id='asdfgh'-->

        <svg style='position:absolute;x=0;y=0;'>
        </svg>
        <div id='table' style='width:90%;height:100%;position:absolute;x=0;y=0;z-index:-1'>
            <smartTable *ngIf=options.showTable [data]=tabledata [settings]=tableOptions></smartTable>
        </div>
    </div>
  `
})
    ...
    createDonut(update){
     ...
     makeTable(){
        ...
        //  console.log('table options ',this.tableOptions);
         for(let j in this.chartData.series)
         {
             temp={};
             var cnt=0;
                for(let i in this.options.name)
                {
                    // console.log(this.tableOptions.columns)
                    this.tableOptions.columns[this.options.name[i] ]={// sets the settings of smart table the columns
                        title: this.options.name[i],
                        filter:false
                    }
                    temp[this.options.name[i] ]=this.chartData.series[j][i];// value for a particular row
                }
